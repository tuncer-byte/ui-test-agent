...
// GEMINI_API_KEY .env dosyasında tanımlı değilse veya çağrı başarısız
// olursa, çağıran taraf (runner.js) kural-tabanlı üreticiye düşer.

async function generateScenariosWithGemini(tool) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    return { ok: false, reason: "GEMINI_API_KEY tanımlı değil (.env dosyasına eklenmeli)" };
  }
